...
const EXERCISE_TYPES = ['mcq', 'gapfill', 'translate', 'order', 'listening', 'matching', 'dialogue', 'errorfix', 'listen-type', 'listen-order'];
const REVIEW_TYPES = ['mcq', 'gapfill', 'translate', 'listening', 'matching', 'dialogue', 'errorfix', 'listen-type'];
  ...
  for (const n of TOPIC_DAYS) {
    ...
    for (const [i, ex] of d.exercises.entries()) {
      ...
      if (ex.type === 'mcq' || ex.type === 'gapfill') {
        ...
        assert.ok(Array.isArray(ex.words) && ex.words.length >= 2, `${d.id} ex${i + 1}: necesita words`);
      ...
      } else if (ex.type === 'listen-type') {
        assert.ok(typeof ex.answer === 'string' && ex.answer.trim().length > 0, `${d.id} ex${i + 1}: answer vacía`);
        assert.ok(ex.audio && ex.audio.trim().length > 0, `${d.id} ex${i + 1}: necesita audio`);
      } else if (ex.type === 'listen-order') {
        assert.ok(ex.audio && ex.audio.trim().length > 0, `${d.id} ex${i + 1}: necesita audio`);
        assert.ok(Array.isArray(ex.words) && ex.words.length >= 2, `${d.id} ex${i + 1}: necesita words`);
        const answer = ex.answer;
        assert.ok(Array.isArray(answer) && answer.length === ex.words.length, `${d.id} ex${i + 1}: answer no es permutación`);
...
test("Contenido: días de examen (7, 14, 21) tienen preguntas y pasan sin 'practice'", () => {
  for (const n of REVIEW_DAYS) {
    ...
    for (const [i, q] of d.review.questions.entries()) {
      assert.ok(REVIEW_TYPES.includes(q.type), `${d.id} preg${i + 1}: tipo inválido '${q.type}'`);
