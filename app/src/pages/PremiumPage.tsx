...
const AI_FEATURES = [
  { icon: Bot, title: 'Tutor IA por texto', desc: 'Conversa con un tutor que corrige tus errores al momento y te da feedback. Hasta 60 mensajes al día.' },
  ...
  { icon: Zap, title: 'Hasta 60 mensajes de IA al día', desc: '20 veces más práctica que el plan Free (3 mensajes al día).' },
...
// Precios SIEMPRE desde la API (/subscription/plans). Sin fallbacks hardcodeados:
// si la API no responde, mostramos "—" en lugar de un precio posiblemente erróneo.
...
export function PremiumPage() {
  ...
  useEffect(() => {
    ...
    getPlans().then((p) => setPlans(p.plans)).catch(() => {});
    // Default: mensual. El anual NO se preselecciona (decisión del usuario).
  }, []);
    ...
    <div className="flex min-h-screen flex-col p-6">
      <div className="mx-auto w-full max-w-md">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-3 text-2xl font-bold">APPINGLES PREMIUM</h1>
          <p className="mt-1 text-sm text-slate-500">Tu aprendizaje personalizado con IA.</p>
        </div>
        ...
        {/* Plan gratuito */}
        <Card className="mt-6">
          <p className="text-xs font-bold tracking-widest text-slate-500">EMPIEZA GRATIS</p>
          <p className="mt-0.5 text-sm text-slate-600">Todo lo necesario para comenzar tu camino.</p>
          ...
          <Button
            ...
            variant="secondary"
            onClick={() => {
              trackAnalyticsEvent('paywall_viewed', { action: 'continue_free' }).catch(() => {});
              navigate('/home');
            }}
          >
            Empezar gratis
        ...
        <Card className="mt-6 border-primary-200">
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              ...
              <p className="text-xl font-black">{fmtUSD(monthly?.price)}<span className="text-xs font-semibold text-slate-400">/mes</span></p>
            ...
            >
              <p className="flex items-center gap-1 text-xs font-semibold text-primary-600">
                Anual <span className="rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">Mejor valor</span>
              </p>
              <p className="text-xl font-black">{fmtUSD(annual?.price)}<span className="text-xs font-semibold text-slate-400">/año</span></p>
              <p className="text-[10px] text-slate-400">{fmtUSD(annual?.pricePerMonth)}/mes equivalente</p>
            </button>
          </div>
